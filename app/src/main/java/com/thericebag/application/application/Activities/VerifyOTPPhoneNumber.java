...
public class VerifyOTPPhoneNumber extends Activity {

    private static final String INTENT_FILTER_FOR_SMS = "android.provider.Telephony.SMS_RECEIVED";
    private RelativeLayout relativeBackground, relativeHeader;
    private TextView txtVerify, txtPhoneNumber;
    private ImageView backIcon;
    private TextView txtResendOTP;
    private EditText editOTP;
    private Context mContext = VerifyOTPPhoneNumber.this;
...
//        getActionBar().hide();
        setContentView(R.layout.activity_verify_otpphone_number);
...
//                startActivity(intent);
    ...
    private void resendOTP() {
        TheRiceBagRequest theRiceBagRequest = new TheRiceBagRequest(mContext, TheRiceBagRequest.Method.GET,
                ApiConstants.ApiName.RESEND_OTP_VERIFY_NUMBER + "email=" + URLEncoder.encode(email) + "&phone=" + phone, new Response.Listener<TheRiceBagResponse>() {
            @Override
            public void onResponse(TheRiceBagResponse response) {
                if (response.isSuccess()) {
                    if (response.getResponseBody().toString().equalsIgnoreCase("1")) {
                        Toast.makeText(mContext, "Success", Toast.LENGTH_SHORT).show(); // TODO Dummy - Have to remove and add functionality
                    } else if (response.getResponseBody().toString().equalsIgnoreCase("0")) {
...
//                txtTime.setText("0:00");
    ...
    private void verifyotp() {
        ...
        TheRiceBagRequest theRiceBagRequest = new TheRiceBagRequest(mContext, TheRiceBagRequest.Method.GET,
                ApiConstants.ApiName.UPDATE_PHONE_VERIFY_OTP + "email=" + URLEncoder.encode(email) + "&otp=" + OTP + "&phone=" + phone, new Response.Listener<TheRiceBagResponse>() {
            @Override
            public void onResponse(TheRiceBagResponse response) {
                ...
                if (response.isSuccess()) {
                    ...
                    if (loginResponse != null && loginResponse.getStatus() != 0 && loginResponse.getStatus() == 1) {
                        ...
                        verifiedOTP(true);
                    } else if (loginResponse != null && loginResponse.getStatus() != 0 && loginResponse.getStatus() == 0) {
                        Toast.makeText(mContext, AppConstants.RESPONSEFAILED, Toast.LENGTH_SHORT).show();
    ...
    private void setScreenHeight() {
        ...
        ViewTreeObserver viewTreeObserver = relativeBackground.getViewTreeObserver();
        viewTreeObserver
                .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        relativeBackground.setLayoutParams(layoutParams);
                        relativeBackground.getViewTreeObserver()
    ...
    private void requestforCallPhonePermission() {
        ...
        if (ContextCompat.checkSelfPermission(mContext, permission)
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(VerifyOTPPhoneNumber.this, permission)) {
    ...
    private void requestForPermission(final String permission) {
        ActivityCompat.requestPermissions(VerifyOTPPhoneNumber.this, new String[]{permission},
    ...
    private void showPermissionRationaleDialog(final String message, final String permission) {
        new AlertDialog.Builder(VerifyOTPPhoneNumber.this)
                .setMessage(message)
                .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        VerifyOTPPhoneNumber.this.requestForPermission(permission);
                    }
                })
